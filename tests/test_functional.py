...
def test_player_buy_spaceship():
    ...
    ships = requests.get(f"{BASE_URL}/station/{totoStationId}/shipyard/list", headers={"Simeis-Key": playerKey}).json()["ships"]
    firstShipId = ships[0]["id"]
    buySpaceshipResponse = requests.post(f"{BASE_URL}/station/{totoStationId}/shipyard/buy/{firstShipId}", headers={"Simeis-Key": playerKey})
...
def test_player_buy_ship_upgrade():
    ...
    ships = requests.get(f"{BASE_URL}/station/{titiStationId}/shipyard/list", headers={"Simeis-Key": playerKey}).json()["ships"]
    firstShipId = ships[0]["id"]
    buySpaceshipResponse = requests.post(f"{BASE_URL}/station/{titiStationId}/shipyard/buy/{firstShipId}", headers={"Simeis-Key": playerKey})
    assert buySpaceshipResponse.status_code == 200

    allPlayer = requests.get(f"{BASE_URL}/gamestats")
    titiMoneyBefore = allPlayer.json()[f"{playerId}"]["money"]

    shipBefore = requests.get(f"{BASE_URL}/ship/{firstShipId}", headers={"Simeis-Key": playerKey}).json()
    cargoCapacityBefore = shipBefore["cargo"]["capacity"]

    buyUpgradeResponse = requests.post(f"{BASE_URL}/station/{titiStationId}/shipyard/upgrade/{firstShipId}/CargoExpansion", headers={"Simeis-Key": playerKey})
    ...
    shipAfter = requests.get(f"{BASE_URL}/ship/{firstShipId}", headers={"Simeis-Key": playerKey}).json()
...
def test_player_buy_heavy_ship_not_enough_money():
    ...
    ships = requests.get(f"{BASE_URL}/station/{tataStationId}/shipyard/list", headers={"Simeis-Key": playerKey}).json()["ships"]
    heavyShip = next(ship for ship in ships if ship["reactor_power"] == 10 and ship["cargo_capacity"] == 1200.0)
    assert heavyShip["price"] > tataMoneyBefore

    buySpaceshipResponse = requests.post(f"{BASE_URL}/station/{tataStationId}/shipyard/buy/{heavyShip['id']}", headers={"Simeis-Key": playerKey})
    assert buySpaceshipResponse.status_code == 200
    ...
    tataPlayer = requests.get(f"{BASE_URL}/player/{playerId}", headers={"Simeis-Key": playerKey}).json()
